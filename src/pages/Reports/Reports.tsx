...
export function Reports() {
  ...
  const { data: leads = [], isLoading } = useLeads({
    ...
    salesExecutive: salesFilter || undefined,
  });
  ...
  function exportCSV() {
    const csv = [
      ...
      ...leads.map((l) =>
        [l.customerCode, `"${l.clientName}"`, l.status, l.contactDetails, l.salesExecutive, l.location, l.phase, l.propertyType, l.preferredSystem, l.proposedCapacity, l.projectValue].join(',')
      ),
    ].join('\n');
  ...
  const installedRevenue = leads
    .filter((l) => l.status === 'TURN_ON')
    .reduce((sum, l) => {
      const val = parseFloat(l.projectValue?.replace(/[^0-9.]/g, '') || '0');
      return sum + (isNaN(val) ? 0 : val);
    }, 0);
  ...
  if (isLoading) {
    return <div className="space-y-4">{[1, 2, 3].map((i) => <div key={i} className="h-48 skeleton rounded-xl" />)}</div>;
  }
    ...
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-1 gap-4 sm:grid-cols-3">
        <div className="stat-card">
          ...
          <div>
            <p className="text-sm text-text-secondary">Total Pipeline</p>
            <p className="text-2xl font-bold text-text">RM {totalRevenue.toLocaleString()}</p>
          </div>
        </div>
        <div className="stat-card">
          ...
          <div>
            <p className="text-sm text-text-secondary">Installed Revenue</p>
            <p className="text-2xl font-bold text-success">RM {installedRevenue.toLocaleString()}</p>
          </div>
        </div>
        <div className="stat-card">
          ...
          <div>
            <p className="text-sm text-text-secondary">Avg Project Value</p>
            <p className="text-2xl font-bold text-text">RM {leads.length > 0 ? Math.round(totalRevenue / leads.length).toLocaleString() : 0}</p>
          </div>
        </div>
      </div>
      ...
      <div className="grid grid-cols-1 gap-6 lg:grid-cols-2">
        <ReportCard title="Leads by Status" icon={BarChart3}>
          {statusCounts.map((s) => (
            <BarRow key={s.value} label={s.label} count={s.count} total={leads.length} />
          ))}
        </ReportCard>

        <ReportCard title="Leads by Sales Executive" icon={TrendingUp}>
          {salesExecs.map((s) => {
            const count = leads.filter((l) => l.salesExecutive === s).length;
            return <BarRow key={s} label={s} count={count} total={leads.length} />;
          })}
        </ReportCard>

        <ReportCard title="Leads by Location" icon={MapPin}>
          {locations.slice(0, 10).map((loc) => {
            const count = leads.filter((l) => l.location === loc).length;
            return <BarRow key={loc} label={loc} count={count} total={leads.length} />;
          })}
        </ReportCard>

        <ReportCard title="Leads by Property Type" icon={Home}>
          {propertyTypes.map((pt) => {
            const count = leads.filter((l) => l.propertyType === pt).length;
            return <BarRow key={pt} label={pt!} count={count} total={leads.length} />;
          })}
        </ReportCard>
      </div>
    </div>
  ...
  return (
    <div className="group">
      ...
      <div className="h-2 rounded-full bg-surface-lighter overflow-hidden">
        <div
          className="h-full rounded-full bg-gradient-to-r from-accent to-emerald-400 transition-all duration-500 group-hover:shadow-lg group-hover:shadow-accent/20"
          style={{ width: `${pct}%` }}
        />
      </div>
    </div>
