...
export function Settings() {
  ...
  async function handleRoleChange(uid: string, role: UserRole) {
    try { await updateRole.mutateAsync({ uid, role }); toast('Role updated successfully', 'success'); }
    ...
    <div className="space-y-6">
      ...
      <div className="card overflow-hidden">
        <div className="border-b border-border px-6 py-4">
          <div className="flex items-center gap-2"><Users className="h-5 w-5 text-text-muted" /><h2 className="font-semibold text-text">User Management</h2></div>
                ...
                <tr key={user.uid} className="table-row">
                  <td className="px-6 py-4">
                    <div className="flex items-center gap-3">
                      <div className="flex h-9 w-9 items-center justify-center rounded-full bg-accent/10 text-sm font-semibold text-accent">{user.displayName?.charAt(0)?.toUpperCase() || '?'}</div>
                  ...
                  <td className="px-6 py-4">
                    <select value={user.role} onChange={(e) => handleRoleChange(user.uid, e.target.value as UserRole)} disabled={updateRole.isPending} className="input-field w-auto min-w-[100px]">
                      <option value="admin">Admin</option><option value="sales">Sales</option><option value="cs">CS</option><option value="engineer">Engineer</option>
                    </select>
                  </td>
                  <td className="px-6 py-4">
                    <button onClick={() => handleActiveToggle(user.uid, !user.active)} disabled={updateActive.isPending} className={`badge cursor-pointer transition-all ${user.active ? 'badge-success' : 'badge-danger'}`}>
